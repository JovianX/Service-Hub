...
class InvitationManager:
    ...
    @staticmethod
    def is_expired(invitation: UserInvitation) -> bool:
        """
        Returns true if invitation is expired and can not be used.
        """
        return is_invitation_expired(invitation.created_at, invitation.expiration_period)

    async def use(self, invitation: UserInvitation):
        """
        Checks and marks invitation as used.
        """
        if invitation.status == InvitationStatuses.pending and self.is_expired(invitation):
